    ...
    <>
      <Container className={`${appStyles.Content} ${styles.Messages}`}>
        ...
        {profileMessages.results.length ? (
          <InfiniteScroll
            children={profileMessages.results.map((message: any) => (
              <Message
                key={message.id}
                {...message}
          ...
          <Asset message={`no messages yet...`} spinner={false} src={false} />
        )}
      </Container>
    </>
  );
  ...
  const mainProfilePosts = (
    <>
      ...
      {profilePosts.results.length ? (
        <InfiniteScroll
          children={profilePosts.results.map((post: any) => (
        ...
        <Col className="py-2 p-0 p-lg-2" lg={8}>
          ...
          {currentUser &&
            ...
            (hasLoaded ? (
              ...
              <Asset spinner src={false} message="" />
            ))}
